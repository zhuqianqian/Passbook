...
public class AccountManager {
    ...
    public class Category {
        ...
        public Category clone() {
            return new Category(mId, mImgCode, mName);
        }
    }
    ...
    private SparseArray<ArrayList<Integer>> mMap;
    private boolean mChanged = false;
    private int mNullCount = 0;

    private static final Collator COLLATOR = Collator.getInstance(Locale.getDefault());
    private static final Comparator<Account> ACC_COMPARATOR = (a0, a1) -> COLLATOR.compare(a0.mProfile, a1.mProfile);
    ...
    List<Category> getCategoryList(boolean includeDefault, boolean sort) {
        List<Category> result = new ArrayList<>();
        for (int i = includeDefault ? 0 : 1; i < mCategories.size(); i++) {
            if (mCategories.valueAt(i) != null) {
                result.add(mCategories.valueAt(i).clone());
            }
        }
        if(sort) {
            Collections.sort(result, (c0, c1) -> COLLATOR.compare(c0.mName, c1.mName));
        }
        return result;
    }
    ...
    private byte[] getCategoryBytes() {
        List<Category> categories = getCategoryList(false, false);
        StringBuilder ret = new StringBuilder("");
        if(categories.size() > 0) {
            for(Category c : categories) {
                ret.append(String.format(Locale.ENGLISH, "%d%s%d%s%s\0", c.mId, FIELD_DELIMITER,c.mImgCode, FIELD_DELIMITER, c.mName));
            }
    ...
    String[] getCategoryNames() {
        ...
        for (int i = 0; i < mCategories.size(); i++) {
            if (mCategories.valueAt(i) != null) {
                names.add(mCategories.valueAt(i).mName);
            }
        }
